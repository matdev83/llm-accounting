...
def test_limit_message_for_project_scope(accounting_for_quota: LLMAccounting):
    ...
    assert message is not None
    assert "PROJECT limit exceeded" in message
    assert f"project '{project_name}'" in message
    # If model is also part of the PROJECT scope limit (which it can be), it might also be in the message.
    # The current QuotaService._evaluate_limits message structure:
    # f"{scope_name} limit exceeded{limit_details}. Max: ... "
    # limit_details includes project if scope is PROJECT. It also includes model if scope is MODEL.
    # It might be good to ensure model is also included in the message if the PROJECT limit was also model-specific.
    # The current UsageLimit model allows a PROJECT scope limit to also specify a model.
    assert f"model '{model_name}'" in message # Check if model detail is included for project-model limit
    assert "1.00 requests" in message
