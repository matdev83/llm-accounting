...
class TestSQLiteAuditLog:
    def test_audit_log_table_creation(self, sqlite_backend: SQLiteBackend):
        """Test that the audit_log_entries table is created with the correct schema."""
        backend = sqlite_backend # Fixture already calls initialize()
        with sqlite3.connect(backend.db_path) as conn:
    ...
    def test_log_single_audit_event(self, sqlite_backend: SQLiteBackend, now_utc: datetime):
        ...
        assert retrieved_entry.timestamp == now_utc.replace(tzinfo=None) # SQLite stores naive, compare as naive or make now_utc naive for comparison

    def test_log_audit_event_minimal_fields(self, sqlite_backend: SQLiteBackend, now_utc: datetime):
        ...
        assert retrieved_entry.log_type == entry_to_log.log_type
        assert retrieved_entry.timestamp == now_utc.replace(tzinfo=None)


    def _log_sample_audit_entries(self, backend: SQLiteBackend, now_utc: datetime) -> List[AuditLogEntry]:
        entries = [
        ...
        # Return entries with original timestamps for comparison (backend converts to naive for SQLite)
        return entries

    def test_get_all_audit_logs(self, sqlite_backend: SQLiteBackend, now_utc: datetime):
        """Test retrieving all audit logs without filters."""
        logged_entries = self._log_sample_audit_entries(sqlite_backend, now_utc)
        retrieved_entries = sqlite_backend.get_audit_log_entries()
        
        assert len(retrieved_entries) == len(logged_entries)
        # Check if IDs are populated and are unique
        retrieved_ids = {e.id for e in retrieved_entries}
        assert len(retrieved_ids) == len(logged_entries)
        assert None not in retrieved_ids

    def test_get_audit_logs_with_date_filters(self, sqlite_backend: SQLiteBackend, now_utc: datetime):
        self._log_sample_audit_entries(sqlite_backend, now_utc)
        
        # Start date only
        retrieved = sqlite_backend.get_audit_log_entries(start_date=(now_utc - timedelta(days=1, hours=1)))
        assert len(retrieved) == 3 # entry2, entry3, entry4

        # End date only
        retrieved = sqlite_backend.get_audit_log_entries(end_date=(now_utc + timedelta(hours=1)))
        assert len(retrieved) == 3 # entry1, entry2, entry3
        ...
        assert len(retrieved) == 2 # entry2, entry3
        app_names = {e.app_name for e in retrieved}
        assert "app2" in app_names
        assert "app1" in app_names # from entry3

    def test_get_audit_logs_with_app_name_filter(self, sqlite_backend: SQLiteBackend, now_utc: datetime):
        self._log_sample_audit_entries(sqlite_backend, now_utc)
        retrieved = sqlite_backend.get_audit_log_entries(app_name="app1")
        assert len(retrieved) == 2
        assert all(e.app_name == "app1" for e in retrieved)

    def test_get_audit_logs_with_user_name_filter(self, sqlite_backend: SQLiteBackend, now_utc: datetime):
        self._log_sample_audit_entries(sqlite_backend, now_utc)
        retrieved = sqlite_backend.get_audit_log_entries(user_name="userA")
        assert len(retrieved) == 2
        assert all(e.user_name == "userA" for e in retrieved)

    def test_get_audit_logs_with_project_filter(self, sqlite_backend: SQLiteBackend, now_utc: datetime):
        self._log_sample_audit_entries(sqlite_backend, now_utc)
        # Test with a specific project
        retrieved = sqlite_backend.get_audit_log_entries(project="proj1")
        assert len(retrieved) == 2
        assert all(e.project == "proj1" for e in retrieved)
        # Test with project=None
        retrieved_none = sqlite_backend.get_audit_log_entries(project=None) # This should filter for project IS NULL
        assert len(retrieved_none) == 1
        assert retrieved_none[0].app_name == "app3" # Entry from app3 has project=None

    def test_get_audit_logs_with_log_type_filter(self, sqlite_backend: SQLiteBackend, now_utc: datetime):
        self._log_sample_audit_entries(sqlite_backend, now_utc)
        retrieved = sqlite_backend.get_audit_log_entries(log_type="prompt")
        assert len(retrieved) == 2
        assert all(e.log_type == "prompt" for e in retrieved)

    def test_get_audit_logs_with_limit(self, sqlite_backend: SQLiteBackend, now_utc: datetime):
        self._log_sample_audit_entries(sqlite_backend, now_utc)
        retrieved = sqlite_backend.get_audit_log_entries(limit=2)
        assert len(retrieved) == 2
        # Default order is DESC by timestamp
        assert retrieved[0].timestamp > retrieved[1].timestamp # Timestamps are naive from DB

    def test_get_audit_logs_with_combined_filters(self, sqlite_backend: SQLiteBackend, now_utc: datetime):
        self._log_sample_audit_entries(sqlite_backend, now_utc)
        retrieved = sqlite_backend.get_audit_log_entries(
            app_name="app1", 
            user_name="userA", 
    ...
    def test_get_audit_logs_empty_result(self, sqlite_backend: SQLiteBackend, now_utc: datetime):
        self._log_sample_audit_entries(sqlite_backend, now_utc)
        retrieved = sqlite_backend.get_audit_log_entries(app_name="non_existent_app")
        assert len(retrieved) == 0

    def test_get_audit_logs_order_by_timestamp(self, sqlite_backend: SQLiteBackend, now_utc: datetime):
        """Verify default order is descending by timestamp."""
        logged_entries = self._log_sample_audit_entries(sqlite_backend, now_utc)
        retrieved = sqlite_backend.get_audit_log_entries()
        
        assert len(retrieved) == len(logged_entries)
        # Timestamps are naive from DB, direct comparison is fine
        for i in range(len(retrieved) - 1):
            assert retrieved[i].timestamp >= retrieved[i+1].timestamp
        
        # Check the most recent original entry (entry4) is first after retrieval
        # Original timestamps: entry1 (oldest), entry2, entry3, entry4 (newest)
        # SQLite stores naive, so convert original for comparison if needed, but they are already naive in this test's helper.
        # The `get_audit_log_entries` converts DB string to naive datetime.
        # Original timestamps are already timezone-aware UTC.
        # The helper stores them as aware, the backend converts to naive string, then get_entries converts back to naive datetime.
        # So, compare retrieved naive with original aware's naive version.
        assert retrieved[0].app_name == "app3" # from entry4 (newest)
        assert retrieved[1].app_name == "app1" and retrieved[1].model == "modelZ" # from entry3
        assert retrieved[2].app_name == "app2" # from entry2
        assert retrieved[3].app_name == "app1" and retrieved[3].model == "modelX" # from entry1 (oldest)
