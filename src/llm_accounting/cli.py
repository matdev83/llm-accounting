...
@click.pass_context
def stats(ctx, period: str, start: datetime, end: datetime):
    ...
    try:
    ...
    except (ValueError, PermissionError, OSError, AttributeError, RuntimeError) as e:
        print(f"Caught handled error in stats: {type(e).__name__}: {e}")
        console.print(f"[red]Error initializing database ({type(e).__name__}): {e}[/red]")
        ctx.exit(1)
    except SystemExit:
        raise
    except Exception as e:
        print(f"Caught unexpected error in stats: {type(e).__name__}: {e}")
        import traceback
        traceback.print_exc()
        ...
        if accounting:
            accounting.__exit__(None, None, None)
...
def track(ctx, model, prompt_tokens, completion_tokens, total_tokens, local_prompt_tokens, local_completion_tokens,
    ...
    try:
        ...
        print(f"Caught handled error in track: {type(e).__name__}: {e}")
        console.print(f"[red]Error initializing database ({type(e).__name__}): {e}[/red]")
        ctx.exit(1)
    except Exception as e:
        print(f"Caught unexpected error in track: {type(e).__name__}: {e}")
        console.print(f"[red]Unexpected error ({type(e).__name__}): {e}[/red]")
        ctx.exit(2)
    finally:
        if accounting:
