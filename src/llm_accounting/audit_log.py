...
class AuditLogger:
    ...
    def log_event(self, app_name: str, user_name: str, model: str, log_type: str,
        ...
        # Ensure self.conn is not None after self.connect() call, which it shouldn't be.
        if self.conn is None:
             raise ConnectionError("Failed to establish database connection.")


        cursor = self.conn.cursor()
        # Use datetime.utcnow() if timestamp is None
    ...
    def log_prompt(self, app_name: str, user_name: str, model: str, prompt_text: str, timestamp: Optional[datetime] = None):
        ...
        # Ensure self.conn is not None after self.connect() call
        if self.conn is None:
             raise ConnectionError("Failed to establish database connection.")
